...
class BlockchainController {
    ...
    validateChain() {
        this.app.get("/chain/validate", async (req, res) => {
            let chainIsValidated = await this.blockchain.validateChain()
            if(chainIsValidated) return res.status(200).send("Validation success");
            else return res.status(500).send("Validation failed");
        })
    }
